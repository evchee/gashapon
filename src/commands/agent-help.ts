...
export default class AgentHelp extends BaseCommand<typeof AgentHelp> {
  ...
  async run(): Promise<void> {
    ...
    const lines: string[] = [
      ...
      'It generates executable wrappers and Claude Code skill files.',
      '',
      '## Config file',
      '',
      `Path: ${cfgPath}`,
      ...
      '### Skills & PATH',
      '```sh',
      'gashapon install-skills                          # write .claude/skills/ for all servers + tools',
    ...
    if (serverEntries.length > 0) {
      ...
      for (const [name, s] of serverEntries) {
        ...
        if (s.installed) lines.push(`  wrapper: \`${wrapperName(name)}\``)
      }
    }
