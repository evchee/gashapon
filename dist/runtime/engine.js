...
export class ExecutionEngine {
    ...
    async execute(serverName, argv) {
        ...
        if (argv.length === 0 || argv.includes('--help') || argv.includes('-h')) {
            ...
            if (helpArgv.length > 0) {
                ...
                if (helpTool) {
                    const helpToolObj = tools.find(t => t.name === helpTool);
                    if (helpToolObj) {
    ...
    outputHelp(serverName, mapping) {
        ...
        lines.push('', 'Discovery:', `  ${bin} --capabilities         Full manifest with all commands and their flags`, `  ${bin} <command> --help       Per-command flag reference`, `  ${bin} schema <command>       Raw JSON schema for a command`, '', 'Global flags:', '  --capabilities   Show full command manifest (JSON)', '  --json           Output as JSON', '  --dry-run        Preview without executing', '  --quiet, -q      Bare output');
        process.stdout.write(lines.join('\n') + '\n');
    }
    outputCommandHelp(serverName, toolName, mapping, tool) {
        const bin = wrapperName(serverName);
        ...
        lines.push('', 'Global flags:', '  --dry-run        Preview without executing', '  --json           Output as JSON', '  --quiet, -q      Bare output');
        process.stdout.write(lines.join('\n') + '\n');
    }
}
