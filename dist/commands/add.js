    ...
    async run() {
        ...
        if (rawSeparatorIdx !== -1) {
            ...
            if (passthrough.length === 0)
                throw usageError('Command required after --');
            ...
            serverConfig = {
                ...
                ...(hasOAuth ? {
                    oauth: {
                        grant_type: flags['oauth-grant-type'] ?? 'authorization_code',
            ...
            if (!flags.command)
                throw usageError('--command is required for stdio transport');
            ...
            throw usageError('Specify transport: use `-- <command>` for stdio, or `--url <url>` for http', ['Example: gashapon add myserver -- npx -y @my/mcp-server',
...
function parseKeyVal(pairs) {
    const result = {};
    for (const pair of pairs) {
        const eqIdx = pair.indexOf('=');
        if (eqIdx === -1)
            throw usageError(`Invalid KEY=VAL pair: "${pair}"`);
